 ...
 * ({@code x()/z()/asLong()}) stays raw so the ChunkMap/DistanceManager/
 * holder registry is keyed consistently. The noise pipeline then operates
 * on small local ints whose real value is {@code epoch + local}. A
 * {@code farlands$epoch} marker field lets the mod detect this jar state.</p>
 */
