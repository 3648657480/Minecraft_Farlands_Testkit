...
## 教训总结（2026-08-18，E1 六连败复盘）

### 铁律一：先写域账本，再动手
每个补丁动笔前必须先回答并**写下来**：这个坐标现在在哪个域（真实/局部/回绕真实），消费者期望哪个域。E1 的 6 轮崩溃全部是混合域错误，而每一轮都在猜域而不是先画域地图。crash-report 里的 "Requested chunk / Generating chunk" 两行 = 域错位的直接证据，**每轮测试前就要预告这两行的含义**。

### 铁律二：先验证"改的代码真的在跑"，再验证行为
- 诊断版 mixin 的 println 必须 `System.out.flush()`，否则冻结时缓冲丢失（v2/v3 诊断没打出来就是这个原因）
- 构建失败时**禁止用旧产物部署**（两次 BUILD FAILED 后照旧复制了陈旧 jar）。部署脚本必须检查构建退出码
- mod 启动打版本标记，**每轮测试第一项 = 用户发版本行**
- 行为级 mixin/patch 必须被 jar 标记门控（`farlands$epoch` 教训：epoch 在 J3 jar 上被激活 → 碰撞全灭 + 渲染空洞，用户发现的，不是测试发现的）

### 铁律三：两次理论失败后必须转测量
冻结排查浪费了 5+ 轮纯理论，真正有效的两步：JFR 方法采样一轮定热点 + 整体跳过一轮二分定位。**同一症状 2 轮理论修复无效 → 停止，二分/仪器化测量。**

### 铁律四：每轮只改一个变量
多轮同时改了客户端 jar + mod + 多个补丁，失败后无法归因。

### 铁律五：服务端/客户端状态同步是隐式依赖
ServerPlayerEpochMixin 未生效（tp 路径没命中目标方法）→ 服务端 epoch 一直为 0、客户端却激活 → 双端状态撕裂比崩溃更隐蔽。凡双端共享状态，第一轮就打日志验证"谁设了、什么时候设的"。
