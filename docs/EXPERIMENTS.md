...
| **1,808,764,368,955,220,493,860,864**（桶 **k0**，≈1.80876436895522×10^24） | **主噪声损毁 onset**：地表冲到接近建造上限、条板墙结构 | `PerlinNoise.wrap()` 的 `+0.5` 误舍入（`2⁸⁸/171.103` 需 +1 上取整才触发折叠） | 本项目条件下**机测**（`topY` 扫描，2 种子一致；见 §8.3） |
| ~~1,808,764,368,950,000,000,099,728~~（旧记录，≈低 19448 ULP） | **作废**（落在 onset 之下，故"起点对不上"） | — | 已由 §8.3 取代 |
...
**与"编年史/外部"记录值的对照**：外部记为 1,808,764,368,955,220,466,364,897；本项目机测 onset
= 桶 **k0** = 1,808,764,368,955,220,493,860,864。两者落在**同一个 double 桶**内
（该量级 ULP = 2²⁸，最小可分辨粒度就是一个桶）→ **1 个 ULP 内一致**。**不作优劣判断**（R10）。
旧记录 1,808,764,368,950,000,000,099,728 低 19448 ULP、在 onset 之下，**已作废**。

### 8.1 "精确起点坐标"是浮动的（不存在普适值）

- **固定层（与种子无关）**：量化档位只由**量级**决定——2^53 起 ulp=2、2^55=8、2^56=16、2^63=2048…
  （double 精度效应，任何种子相同）。
- **浮动层（与种子绑定）**：某量级上"看不看得见条带/墙结构"取决于**噪声场**，而噪声场由**种子**决定
  → **视觉上的"起点坐标"随种子浮动**。但要区分：**主噪声损毁 onset 是固定的**（桶 k0，与种子无关，
  见 §8.3）；随种子浮动的只是级联里**更早、更弱的层**（约 2⁶⁰–2⁷⁵ 的地表条带/变平）。
...
### 8.3 起点机测定案（2026-09-24，2 种子）

**判据**（可机测，不靠目视）：主噪声损毁后地表冲到接近建造上限；读 testgen 的 `topY`。
**方法**：`-Dfarlands.spawnset=<坐标>,100,<坐标>` 设 epoch → 生成 (0,0)/(62,62) → 读 `topY`。

| 坐标 | seed 12345 | seed -7820689566140440746 | seed -7544029881582320318 |
|---|---|---|---|
| 1.3e24 / 1.5e24 / 1.7e24 / 1.805e24 | 62 | 62 | 62 |
| 旧记录 `1.80876436895000e24` | 62 | – | – |
| 桶 k0−2 `1808764368955219956979456` | – | – | 62 |
| 桶 k0−1 `1808764368955220225425408` | 62 | 62 | 62 |
| **桶 k0 `1808764368955220493860864`** | **252** | **251** | **252** |
| 2^81 / 2^85 / 2^90 | 252 | 252 | 252 |

> **3 种子复核（2026-09-27，seed `-7544029881582320318`）**：k0−2 / k0−1 / 1.5e24 → `topY=62`（正常）；
> k0 / 2^81 → `topY=252`（损毁）。k0 的 WG 指纹示例：`(0,0) surf=ccaf8f77f456 floor=945e397591c5`、
> `(62,62) surf=3e451c4ee96e floor=6fc9e118b055`，`biome=71776462bb38`。
> 即 **onset 在 k0 处 1 ULP 突变、3 种子一致、可复现**。**注意**：`topY` 只捕捉**主噪声损毁**；
> 更早、更弱的"肉眼条带/变平"层（约 2⁶⁰–2⁷⁵）**比 k0 早且随种子浮动**，不在此机测指标内。

- **结论**：转折在 **1 个 ULP 内**、正好桶 **k0**、**两种子一致** → 主噪声 onset 固定、不随种子变。
- 旧记录低 19448 ULP、在 onset 之下 → **作废**；onset = k0，与编年史/INF 同一 double（ULP=2²⁸）。
- 理论式 `2⁸⁸/171.103` 落桶 k0−1（距 k0−1/k0 分界 0.0103 ULP）→ 需 **+1 上取整** 才触发折叠。
- 更早的"地表条带/变平"（约 2⁶⁰–2⁷⁵）是级联里更早的弱层、随种子变，**与主噪声损毁无关**。
- 证据：`exp-SCANA*-*.log`、`exp-SCANB-*.log`、`exp-NARROW-*`。
- **不构成对外部记录的否定/更正**（R10）。

### 8.4 可见「奶酪墙」的实测位置（2026-09-27，客户端）

> **修正 §8.3 的误导**：k0 是**机测**主噪声 onset（`topY` 突变点），**但客户端在 k0 附近看不到“墙”**
> ——k0 epoch 内（local 0 附近）是**一整片 256 平台**（扫 local 0→1e8：topY 全 252、floor 指纹相同）。

**可见的「奶酪墙」（62↔256 / 漂浮板地形）在 epoch 的 local 边沿**：

| 项 | 值 |
|---|---|
| 真实 X | **`1808764368955220359643137`** |
| local X | `−134217727` = **−(2²⁷−1)** |
| epoch | `1808764368955220493860864`（= k0） |
| 关系 | 墙 = **epoch − (2²⁷−1)** = **epoch − 134,217,727** |
| 观测 | MC 26.2 客户端，fork jar + 1.0.1；`/realtp`；F3 `Local (in-epoch) = (−134217727, 256, 0)`，地形为漂浮板/奶酪 |

- **想看“边境之地（奶酪墙）”→ 去 `1808764368955220359643137`，不是 k0**
  （k0 只有一片 256 平台、没有可见的墙——去 k0 只会看到平台/空气）。
- **它会“动”的原因**：墙在 **local ±2²⁷**（local 域边界），真实坐标 = **epoch ± 2²⁷** → 换 epoch 就换位置。
- **复现**：先 `/realtp 1808764368955220493860864 100 0`（把 epoch 设成 k0），再
  `/realtp 1808764368955220359643137 100 0`。
- 证据：F3 截图（2026-09-27）。
- **R10**：本项目条件下的特定结果。
