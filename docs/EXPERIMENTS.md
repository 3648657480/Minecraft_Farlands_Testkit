...
## 8. 远域现象表（实测，2026-09-23）

| 坐标（真实） | 现象 | 机制 | 验证 |
|---|---|---|---|
| 2^53 = 9,007,199,254,740,992 | 症状起点：1 格分辨率采样量化为 2 格成对；地表正常、地下细微 | double ulp = 2 | 已测试 |
| 2^54 = 18,014,398,509,481,984 | 量化步长 = 采样格宽（4 格），仍以自然形态为主 | ulp = 4 | 已测试 |
| 2^55 = 36,028,797,018,963,968 | 主地形量化：规则阶梯/条带（奶酪状） | ulp = 8 > 格宽 4 | 已测试 |
| 2^56 = 72,057,594,037,927,936 | 平板状结构 + 水面瓷砖（16 格） | ulp = 16 | 已测试 |
| 2^63 = 9,223,372,036,854,775,808 | 同质拼图（128 chunk 均匀块） | ulp = 2048 | 已测试 |
| **1,808,764,368,950,000,000,099,728**（≈1.80876436895×10^24） | **64 位边境之地起点：主地形/生物群系改变，长条水平板/墙结构** | `PerlinNoise.wrap()` 取整失效（`p/2^25+0.5` 的 +0.5 在高值处丢失） | **实测（人工二分）** |
| >2.43×10^27 | 地形**未**终止，仅高度重复（Wiki 的终止值不适用） | — | 实测 |

**级联性质（重要）**：不存在单一"起点"——各噪声 wrap 失效阈值不同，按层先后崩坏：
- 地下（JAGGED ~2×10^20、CAVE_CHEESE ~2.3×10^23、SHIFT ~3.0×10^23）**先坏**
- 地表主地形（CONTINENTALNESS 一带）到 ~1.8088×10^24 才改变
- 因此"边境之地起点"应记录为分层阈值，而非单点

**与 Wiki 值的差异**：Wiki 记 1,808,764,368,955,220,466,364,897；实测 1,808,764,368,950,000,000,099,728。
两者相差 5.22×10^12 格（≈19,450 ulp）——同一边界带，实测值更精确（Wiki 偏上）。

**F3 显示（v0.5.2+ 修复）**：XYZ/Block/Chunk 显示真实坐标（超 double 精度用 BigInteger 精确值）；
