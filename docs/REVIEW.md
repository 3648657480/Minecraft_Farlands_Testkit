...
## 3. 精度现象坐标表（本项目实验条件下的特定结果，红线 R10）

| 坐标 | ulp（格） | 现象 |
|---|---|---|
| 2^52（4.5e15） | 1 | 完全正常 |
| **2^53（9.007e15）** | 2 | 症状起点：1 格采样量化为 2 格成对（地下细微） |
| 2^55（3.603e16） | 8 | 主地形量化：8 格阶梯/条带（奶酪状） |
| 2^56（7.2e16） | 16 | 平板结构 + 水面瓷砖 |
| **2^63（9.223e18）** | 2048 | 128 chunk 同质拼图 |
| ~1.80876436895e24 | ~2^28 | 本项目条件下测得的"地表主地形改变"起点 |
| >2.43e27 | — | 本条件下地形未终止（高度重复） |
| 1e306 | ~1e290 | 完全冻结 |
| 1.8e308（double MAX） | ~2^1024 | 水柱世界（水平塌缩+垂直正常+海洋填水） |

远域退化是**级联**：不同噪声在不同阈值失效（地下先、地表主地形后），无单一"起点"。
