...
禁止：声称推翻、纠正或否定社区/官方记录（如 Minecraft Wiki）；把本项目结果表述为"更准确"或"通用结论"。

原因：外部记录基于其自身的实验条件（版本、实现、测量方法）；本项目数据来自自研管线（epoch/local 域），两者不可直接比较。越界声称既不尊重，也不严谨。

验证：已执行（EXPERIMENTS.md 现象表按"特定实验条件下的特定结果"表述，2026-09）。

处置：无（写作准则）。所有结果必须写明实验条件，并声明不构成对外部记录的否定或更正。
