...
**直接 `/realtp <x> 100 0` 传过去即可**（先把渲染距离降到 4–6）。以下坐标在本管线
**与种子无关、可复现**：
...
| **边境之地「奶酪墙」（想看的那个）** | **`1808764368955220359643137`**（= k0 − 2²⁷+1，epoch 边沿） |
| 1e306 / double 极限 | `1e306` |

> ⚠️ **别去 k0（`1808764368955220493860864`）找墙**：那是“主噪声 onset”的机测点，
> 但在客户端那里只有**一整片 256 平台**（没有可见的墙/边界）。**可见的奶酪墙在
> `1808764368955220359643137`**（= k0 − (2²⁷−1)）。详见 [EXPERIMENTS.md](EXPERIMENTS.md) §8.4。

- 上面这些是**量级 / 桶阈值**，**与种子无关、无需二分**。机测（3 个种子）显示：k0 处
  地表 1 ULP 内直接突变（`topY` 62 → 252），3 种子一致（见 [EXPERIMENTS.md](EXPERIMENTS.md) §8.3）。
- **只有**“级联里更早、更弱的肉眼条带 / 变平层”（约 2⁶⁰–2⁷⁵）**才**随种子浮动——那是**研究用途**，
  不是“看边境之地”所必需；要研究它再做二分（[EXPERIMENTS.md](EXPERIMENTS.md) §8.1/§8.2）。
- 表述原则（R10）：本篇数值均为本项目实验条件下的特定结果。
